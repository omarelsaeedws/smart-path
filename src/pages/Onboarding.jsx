        ...
        <div className="text-center mb-8">
          <div className="inline-flex items-center justify-center w-20 h-20 rounded-3xl bg-gradient-to-tr from-sky-400 to-indigo-500 shadow-xl shadow-sky-500/30 mb-6 rotate-6">
            <FontAwesomeIcon
              ...
              className="text-white text-3xl -rotate-6"
            />
          </div>
          <h1 className="text-3xl font-extrabold text-slate-900 dark:text-white mb-2">
            مرحباً بك في SmartPath
          </h1>
          <p className="text-slate-500 dark:text-slate-400">
            أخبرنا عنك لنخصص تجربتك
          </p>
        </div>
          ...
          {steps.map((s, i) => (
            <React.Fragment key={i}>
              <div className="flex flex-col items-center gap-1">
                ...
                <span
                  className={`text-xs font-medium ${step === i + 1 ? "text-sky-600 dark:text-sky-400" : "text-slate-400 dark:text-slate-500"}`}
                >
                  {s}
                </span>
              </div>
              {i < steps.length - 1 && (
                <div
                  className={`flex-1 h-0.5 mb-5 transition-all ${step > i + 1 ? "bg-green-500" : "bg-slate-200 dark:bg-slate-700"}`}
                />
              )}
            </React.Fragment>
          ))}
          ...
          {step === 1 && (
            <div>
              ...
              <p className="text-slate-500 dark:text-slate-400 text-sm mb-6">
                سنختار لك مسارات مناسبة بناءً على مستواك
              </p>
          ...
          {step === 2 && (
            <div>
              ...
              <p className="text-slate-500 dark:text-slate-400 text-sm mb-5">
                اختر اهتماماً واحداً أو أكثر من المجالات المتاحة
              </p>

              {catsLoading ? (
                <div className="flex justify-center py-10">
                  <p className="text-slate-400 dark:text-slate-500 text-sm animate-pulse">جاري تحميل الفئات...</p>
                </div>
              ) : categories.length === 0 ? (
                <div className="text-center py-8">
                  <FontAwesomeIcon
                    icon={faLayerGroup}
                    className="text-slate-300 dark:text-slate-600 text-4xl mb-3"
                  />
                  <p className="text-slate-500 dark:text-slate-400 text-sm">
                    لا توجد فئات متاحة بعد. تواصل مع المدير.
                  </p>
                </div>
              ) : (
                <div className="grid grid-cols-2 gap-3">
                  {categories.map((cat, idx) => {
                    ...
                    return (
                      ...
                      >
                        <div
                          className={`w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 bg-gradient-to-br ${selected ? "from-sky-400 to-indigo-600" : gradient} opacity-${selected ? "100" : "80"}`}
                        >
                        ...
                        <div className="flex-1 min-w-0">
                          <span className="text-slate-800 dark:text-white font-medium text-sm line-clamp-1">
                            {cat.name}
                          </span>
                        </div>
                        {selected && (
                          <FontAwesomeIcon
                            icon={faCheckCircle}
                            className="text-sky-500 dark:text-sky-400 text-sm flex-shrink-0"
                          />
                        )}
                      </button>
                    );
              ...
              {interests.length > 0 && (
                <p className="text-sky-600 dark:text-sky-400 text-xs text-center mt-3 font-medium">
                  ✓ تم اختيار {interests.length}{" "}
                  {interests.length === 1 ? "اهتمام" : "اهتمامات"}
                </p>
              )}
              {error && (
